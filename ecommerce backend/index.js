    ...
    if (err) throw err;
      ...
      if (req.session.loggedin) {
        ...
        for (let i = 0; i < result.length; i++) {
          ...
          let Squery = "delete from cart where username='"+req.session.username+"' and p_id="+result[i].p_id;
          let q = conn.query(Squery,(err,results)=>{
              if(err) throw err;  
          })
...
app.get("/", function (req, res) {
  let sqlQuery =
    "SELECT types.type,p.id,p.name,p.des,p.price,p.sale_price FROM types INNER JOIN products p ON p.type = types.id";
  let query = conn.query(sqlQuery, (err, results) => {
    if (err) throw err;
    else {
